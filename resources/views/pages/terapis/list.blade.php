...
<div class="col-lg-6 col-7">
  <h6 class="h2 text-white d-inline-block mb-0">Terapis</h6>
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      <li class="breadcrumb-item"><a href="#"><i class="fas fa-hands-helping"></i></a></li>
      <li class="breadcrumb-item active" aria-current="page">Terapis</li>
    </ol>
  </nav>
</div>
@endsection
...
<script>
  let _url = {
    create: `{{ route('terapis.create') }}`,
    edit: `{{ route('terapis.edit', ':id') }}`,
    show: `{{ route('terapis.show', ':id') }}`,
    destroy: `{{ route('terapis.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["terapis-table"].draw()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
