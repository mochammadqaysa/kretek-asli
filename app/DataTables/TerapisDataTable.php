...
class TerapisDataTable extends DataTable
{
            ...
            ->orderColumn('cabang', function ($query, $direction) {
                $query->orderBy(
                    Cabang::select('cabang.nama')
                        ->whereColumn('cabang.uid', 'terapis.cabang_uid')
                        ->limit(1),
                    $direction
                );
            })
     ...
     *
     * @param \App\Models\Terapi $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Terapis $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (PermissionCommon::check('role.create')) {
            $button[] = Button::raw('<i class="fa fa-plus"></i> Tambah Terapis')->action('function() { create() }');
        }
        return $this->builder()
            ->parameters([
                'language' => [
    ...
    public function getColumns(): array
    {
        ...
        $column[] = Column::make('cabang')->addClass('text-wrap');
        return $column;
    }
